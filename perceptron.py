...
highest_acc = 0.0
for n_el in n: #iterates over n

    for r_el in r: #iterates over r
        ...
        for alg in ['Perceptron', 'MLP']: #iterates over the algorithms
            ...
            #-->add your Pyhton code here
            if alg == 'Perceptron':
                clf = Perceptron(eta0=n_el, shuffle=r_el, max_iter=1000)
            else:
                clf = MLPClassifier(activation='logistic', learning_rate_init=n_el, shuffle=r_el, max_iter=1000)
            ...
            for (x_testSample, y_testSample) in zip(X_test, y_test):
                ...
                correct.append(1 if prediction == y_testSample else 0)
            ...
            if acc > highest_acc:
                highest_acc = acc
                print("Highest " + alg + " accuracy so far: " + str(highest_acc) + ", Parameters: learning rate=" + str(n_el) + ", shuffle=" + str(r_el))
