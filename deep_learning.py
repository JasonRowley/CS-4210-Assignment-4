...
for h in n_hidden:                          #looking or the best parameters w.r.t the number of hidden layers
    for n in n_neurons:                      #looking or the best parameters w.r.t the number of neurons
        for r in l_rate:                  #looking or the best parameters w.r.t the learning rate
            ...
            model = build_model(h, n, len(class_names), r)
            #-->add your Pyhton code here
            ...
            #-->add your Pyhton code here
            class_predicted = np.argmax(model.predict(X_test), axis=-1)
            correct = [1 if pred == ground else 0 for pred, ground in zip(class_predicted, y_test)]
            accuracy = sum(correct) / len(class_predicted)
            if accuracy > highestAccuracy:
                highestAccuracy = accuracy

            print("Highest accuracy so far: " + str(highestAccuracy))
            print("Parameters: " + "Number of Hidden Layers: " + str(h) + ",number of neurons: " + str(n) + ",learning rate: " + str(r))
            print()
